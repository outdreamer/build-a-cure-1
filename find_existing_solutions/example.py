...
'''
Workflow 1: Identify problem metadata (like type), query for solutions to adjacent problems, transform into adjacent problem if found, solve, and convert to original problem
...
'''
Workflow 7: Convert to pattern interface (solution patterns, problem patterns, problem-solution matching patterns) starting with known patterns for this problem type & core functions of solutions.
...
'''
Workflow 10: Identify (abstract/contextual) interface combination that will prove most useful for solving this problem, frame problem on those interfaces, and use interface queries to find solution query, then convert solution query to structural-informational solution implementation.
...
'''
Workflow 11: Derive & describe system in which problem occurs using system analysis (identify core functions, vertices, conflicts, structures (layers/limits/gaps), variance shapes & sub-systems) and interface query for that system, then solve problem by adjusting system.
