to do: organize into separate docs
  ...
  - the roots (intersections at zero) are the different types of transform (addition & subtraction of scaled base versions) positioned as equivalents
    - the implied question is:
      ...
      - or "how do you equate these two vectors (addition/subtraction set constants) with the fewest numbers (with one number, in the case of a one-variable polynomial)"
      - or "how do you combine the concept of area related to points, lines, squares, cubes, etc to generate these vector sets"
        ...
        took the square root of that product    (find number to produce product with one multiplication (squared))    find alternate generative factor (specifically square root) of the product of that multiplication
        "

      - youre doing a similar operation twice - first create alternate output of factors (roots), then find alternate factors (roots vs. the final square root)
